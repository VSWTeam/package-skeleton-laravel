...
```bash
php artisan vendor:publish --provider="Vswteam\Skeleton\SkeletonServiceProvider" --tag="migrations"
php artisan migrate
```

You can publish the config file with:
```bash
php artisan vendor:publish --provider="Vswteam\Skeleton\SkeletonServiceProvider" --tag="config"
```
...
echo $skeleton->echoPhrase('Hello, Vswteam!');
```
